...
def main():
	figure_path=sys.argv[1];path_out=sys.argv[2];
	makedir(path_out)
	locPath=os.path.join(path_out,'filter_stat_image')
	colorPath=os.path.join(path_out,'eyebrow_image')
	pointNumPath=os.path.join(path_out,'point_number')
	makedir(locPath);makedir(pointNumPath);makedir(colorPath);
	img_list=getImgList(figure_path)
	thNum=5#num of threshold levels
